...
The eMBB service fulfills the demand for high capacity and throughput. Moreover, the URLLC service offers autonomous vehicles, tactile internet, remote surgery services, and high quality and low latency services. However, mMTC comprises a large number of internet of things (IoT) devices that transmit small payloads
\cite{shen2020ai,setayesh2020joint,popovski20185g,dogra2020survey,kassab2018coexistence,alsenwi2021intelligent}.


This paper considers several user equipments (UEs) requesting one of these types of services with different QoS. We would like to study RAN slicing technology and the isolation between slices in this system to evaluate the performance of various services.
...
\textcolor{MidnightBlue}{The concept of network function virtualization (NFV) refers to the separation of network software and hardware elements. Therefore network functions can run on commodity hardware.} Virtual network functions (VNF) are system function blocks in NFV systems. The NFV technology offers to execute VNFs as virtual machines or containers on a cloud environment \cite{mijumbi2015network, luo2020online}.
\textcolor{MidnightBlue}{As a result, some O-RAN components, such as user plane function (UPF), O-CU, O-DU, and RAN Intelligent Controller (RIC)-near real-time, are virtualized and implemented as VNFs. VNFs can also be deployed as virtual machines (VMs) or containers.}
...
\item We consider the processing delay and the VNF resources needed for the slice compared to other papers. Therefore, we focus on obtaining the optimal number of VNFs in each layer of the O-RAN architecture. 
\item \textcolor{MidnightBlue}{Various QoS conditions is considered, including delay, power, and throughput. According to the number of activated VNFs and the throughput of the UEs, we calculate the end-to-end processing delay of the system.
Assuming a limited fronthaul capacity, we calculate each O-RU's power and capacity. Depending on the service type, we model the interference of neighboring O-RUs and determine the throughput. The Shannon capacity theorem cannot account for URLLC and mMTC because they are short packet transmissions.}
\item The main problem is mixed-integer non-linear programming that is difficult to solve.
We perform a two-step iterative algorithm to solve it.
The algorithm is presented for resource management with the first-step VNF activation, power allocation, PRB association, and the second-step O-RU association.
\item We reformulate and simplify the main problem for the first step of finding an upper and lower bound for the number of activated VNFs and using lagrangian function and KKT conditions to find optimal power and PRB allocation.
