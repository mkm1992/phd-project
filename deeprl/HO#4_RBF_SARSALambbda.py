...
# Pronlem 1 part A : RBF state coding _ SARSA(Lambda)
#-------------------------------
import gym
...
class RL:
    def __init__(self, env,actions,d1,d2):
        ...
        self.epsDecay = 0.995
        self.W = np.zeros((len(actions),self.d1*self.d2))
        self.RewardSum = []
    # multivariate RBF Function
    def RBF(self,s):
        x = np.zeros([self.d1*self.d2,1])
        mean = list()
        cov = list()
...
#                self.alpha = max(0.0, self.alpha * self.alphaDecay)
            ...
            w = w.reshape(self.d1*self.d2, 1)  
            w = w + self.alpha * (delta + Q - Q_old) * Z - self.alpha * \
                                                       (Q - Q_old) * x
            self.W[self.actions.index(chosen_action)] = w.reshape(self.d1*self.d2)
            Q_old = Q_prime
            x = x_prime
...
for i in range(100, numEpisodes):
    avgReward.append(np.mean(Episode.RewardSum[i - 100:i]))
fig, ax = plt.subplots(figsize = (18, 8))
plt.plot(avgReward)
plt.yticks(range(-110, -210, -10))
