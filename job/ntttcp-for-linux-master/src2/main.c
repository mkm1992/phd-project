...
/************************************************************/
int run_ntttcp_sender(struct ntttcp_test_endpoint *tep, char *th_result)
{
	 ...
	 *th_result = print_test_results(tep);

	return err_code;
}

int run_ntttcp_receiver(struct ntttcp_test_endpoint *tep, char *th_result)
{
	int err_code = NO_ERROR;
	struct ntttcp_test *test = tep->test;
	char *log = NULL;
...
struct group func1()
{
	struct group result1;
	int argc1 =1;
	char **argv1;
	const char* n_argv[] = { "r" };
    argv1 = n_argv;
	result1.err_code = NO_ERROR;
	cpu_set_t cpuset;
	...
	result1.err_code = parse_arguments(test, argc1, argv1);
	if (result1.err_code != NO_ERROR) {
	...
	result1.err_code = verify_args(test);
	if (result1.err_code != NO_ERROR) {
	...
	if (test->client_role == true) {
		...
		result1.err_code = run_ntttcp_sender(tep, result1.th_result);
	}
	else {
		tep = new_ntttcp_test_endpoint(test, ROLE_RECEIVER);
		result1.err_code = run_ntttcp_receiver(tep,result1.th_result);
	}
