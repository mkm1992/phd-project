...
def extract(path, tolerance=DEFAULT_TOLERANCE,ratio=DEFAULT_RATIO, limit=None):
    #pixels = load(path)
    img = Image.open(path)
    img = img.convert("RGB")
    pixels= list(img.getdata())
    lenimag=np.shape(img)
    print(lenimag)
    threshold=lenimag[0]*lenimag[1]*ratio
    counter = count_colors(pixels)
    tmp = dict()
    for color, count in counter.items():
        tmp[colorutil.rgb_lab(color)] = count

    counter = compress(tmp, tolerance,threshold)
...
def compress(counter, tolerance,threshold):
    result = counter
    if tolerance <= 0:
        return result
    ...
    while i < len(colors):
        ...
        j = i + 1
        while j < len(colors):
            tolerance = toleranceprev
            smaller = colors[j]
            dist=colorutil.cie76(smaller, larger)

            if result[larger] < threshold:
                tolerance =min(5*tolerance, 100)
                print(tolerance)
            #print(tolerance)
            if dist < tolerance:
