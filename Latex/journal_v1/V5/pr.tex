...
Finding a feasible initial value is almost tricky. To overcome this challenge, we compare it with the baseline scheme method using the CVX of Matlab which used successive convex approximation (SCA) method. If the baseline scheme converged and has a feasible solution, so our algorithm (IABV) can be converged too.
...
In figure 12, the aggregate throughput is shown according to the number of UEs for two different methods, namely the
proposed algorithm (IABV) and the optimal method for URLLC service.
