...
In this section, numerical results for the main problem are depicted to evaluate the performance of the algorithms using the Monte-Carlo method. We consider three network slices for eMBB, URLLC, and mMTC services.
...
\begin{figure}
  ...
  \label{fig:cell}
\end{figure}
Here, The channel vector from the O-RU $r$ to the UE $i$ in service $s$ is set as $\bold{h}_{r,u(s,i)}^{k}=d_{r,u(s,i)}^{-\mathcal{L}} \Omega_{r,u(s,i)}^{k} $.  Where, $d_{r,u(s,i)}^{-\mathcal{L}} $ is the distance between the O-RU $r$ and the UE $i$ in service $s$ and $\mathcal{L} = 3.8$ is the path-loss exponent.
...
Finding a feasible initial value is almost tricky. To overcome this challenge, we compare it with the baseline scheme method using the CVX of Matlab. If the baseline scheme converged and has a feasible solution, so our algorithm (IABV) can be converged too.
...
Assume each UE in each mMTC service instance requires 1bits/sec/Hz data rate and is not sensitive to the end-to-end delay. Also, suppose the fronthaul link has no restrictions. The figure depicts that by increasing the number of UEs in each instance of the service, the aggregate throughput increases. 
...
The minimum data rate for each UE in this service is 6 bits/sec/Hz. 
...
\begin{figure}
  \centering 
    \includegraphics[scale = 0.4]{cloudVsFronthaul.eps}
    %\includegraphics[max height=30cm,max width=9.5cm]{Drawing15.eps}
    %\includegraphics[width=\textwidth]{finalDraw.pdf}
  \caption{Aggregate Throughput vs. Number of UE in a Service}
  \label{fig:10}
\end{figure}
