...
During the simulation, it can be noted that sometimes the algorithm does not converge in a number of these simulations for a certain number of UEs.
For a fixed number of UEs, by comparing convergent and non-convergent simulations, we experimentally found that in cases where convergence does not occur, UEs are at the edge of the boundaries or far away from the O-RU and have a weak channel gain. One solution is to eliminate UEs who undermine system convergence.
...
Moreover, the allocation of physical resources to users is based on dividing the UE number by the total number of PRBs.
If the number of UE is more than the resources, each UE is given exactly one PRB without interference, otherwise the amount of interference will be very low.
