...
which offers services with special needs. Multiple network Slices in the same infrastructure run and manage
...
for different type of 5G services includes URLLC, eMBB and mMTC services which contain different application instances.

In this paper, resource allocation in an O-RAN system is considered using the concept of network slicing.
Here, we optimize the baseband resources and the VM activation in O-RAN components base on their service priority for different services to maximize sum rate of system.
The problem of baseband resources includes o-RU assignment, physical resource block (PRB) and power allocation. The main problem is a mixed integer non-linear programming  
\end{abstract}
