...
We solve this problem in two-level iteratively until it converges. In the first level, parameters ($\boldsymbol{P}, \boldsymbol{E}, \boldsymbol{M}$) are obtained by relaxing and reformulating parameters and turn them into a convex problem; Afterward, we solve it by dual optimization problem.
...
Figure \ref{fig:8}, represents the aggregate throughput concerning the number of UEs and the maximum power for three different mMTC services. mMTC services include a large number of UEs with low data rates and low power.
Assume each service instance requires $1bits/sec/Hz$ data rate and is not sensitive to end-to-end delay. The figure depicts that by increasing the number of UEs in each instance of the service, the aggregate throughput increases. Also, by increasing the maximum power of each UE in each instance of mMTC service, the aggregate throughput rises too.
\begin{figure}
  %\centering 
