...
\begin{equation}
\begin{split}
T^{tot} &=  T^{process} + T^{transmission} + T^{propagation}\\
T^{process} &=  T^{RU} + T^{DU} + T^{CU} + T^{UPF}\\
T^{transmission} &= T^{front} + T^{mid} + T^{back} + T^{trans2net} \\
T^{propagation} &= T^{front} + T^{mid} + T^{back} + T^{trans2net} \\
\end{split}
\end{equation}
%T_{tot} = T_{RU} + T_{front} + T_{DU} + T_{mid} + T_{CU} + T_{back} + T_{core} + T_{trans2net}
The total delay ($T^{tot}$) is the sum of the processing delay ($T^{process}$), the transmission delay ($T^{transmission}$), and the propagation delay($T_{propagation}$). 
...
\begin{equation}
T^{process}_{u(s,i)} =  T^{RU}_{u(s,i)} + T^{DU}_{s} + T^{CU}_{s} + T^{UPF}_{s}
\end{equation}
Hence, $T^{tot}_{u(s,i)} \approx T^{process}_{u(s,i)} $
...
\eqref{c11}, and \eqref{c12}, indicate that the power of each O-RU does not exceed the maximum power, and the power of each UE is a positive integer value, respectively.  
