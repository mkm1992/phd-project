...
Central Unit (O-CU). 
Some O-RAN components that includes user plane function (UPF) , O-CU, O-DU are virtualized and implemented as a virtual network function VNF that can be run on virtual machines (VMs) or containers.
In this paper, we study the problem of baseband resource allocation and the VM activation in O-RAN architecture based on their service priority for different type of 5G services includes enhanced mobile broadband (eMBB), ultra-reliable low latency communications (URLLC) and massive Machine Type Communications (mMTC) services.
...
The proposed method is confirmed by the simulation results.  
\end{abstract}

\begin{IEEEkeywords}
Open Radio Access Network (O-RAN), Virtual Network Function (VNF), Virtual Machine (VM)
\end{IEEEkeywords}
