...
So the the aggregate throughput is ascending function concerning the number of iterations and it will converge to the optimal solution.
...
Assume each UE in each mMTC service instance requires $1bits/sec/Hz$ data rate and is not sensitive to the end-to-end delay. Also, suppose the fronthaul link has no restrictions  .The figure depicts that by increasing the number of UEs in each instance of the service, the aggregate throughput increases. Also, by increasing the maximum power of each UE in each instance of mMTC service, the aggregate throughput rises too.
\begin{figure}
  %\centering 
