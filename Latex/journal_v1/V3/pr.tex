...
Assume we have preallocated slices serving these services;
So each service request $s$ served by its corresponding slice.
Each Service $s\in \{1,2,3 \} $ consists of $U_{s}$ request from the 
single-antenna UEs which require certain QoS to be able to use the requested program indicate service type).
...
Each slice $s_ \in \{1,2,3 \}$ consists of $K_{s}$ preallocated virtual resource blocks that are mapped to Physical Resource Blocks (PRBs), $M_s^{d}$ VNFs for the processing of O-DU, $M_s^{c}$ VNFs for the processing of O-CU-UP and $M_s^{u}$ VNFs for the processing of UPF.
...
In addition, there are $R$ single-antenna RU that are shared between slices. 
All RUs $\{ r_i | i\in 1,2,...,R \}$
transmit signals cooperatively to all the UEs. 
Also $\mathcal{R} = \{ r_i | i\in 1,2,...,R \}$ depicts the set of o-RUs.
Moreover, all o-RUs, have access to all PRBs.
\subsection{The Achievable Rate}
The SNR of $i^{th}$ UE  served at slice $s$ on PRB $k$ is obtained from
\begin{equation}\label{eq2}
\rho_{u(s,i)}^{k} =  \frac{|p_{u(s,i)}^{k}{\bold{h}_{\mathcal{R},u(s,i)}^{H \: k}} \bold{w}_{\mathcal{R},u(s,i)}^{k}}{BN_0 + I_{u(s,i)}^{k}},
\end{equation} 
where $p_{u(s,i)}^{k}$ represents the transmission power allocated by set of o-RUs $\mathcal{R}$ to $i^{th}$ UE requesting $s^{th}$ service on PRB $k$. 
${\bold{h}_{\mathcal{R},u(s,i)}^{k}} \in \mathbb{C}^{R}$ is the vector of channel gain of a wireless link from set of 
$\mathcal{R} = \{r\in 1,2,...,R\}$ O-RUs to the $i^{th}$ UE requesting $s^{th}$ service. In addition, $\bold{w}_{\mathcal{R},u(s,i)}^{k} \in \mathbb{C}^{R}$ depicts the  transmit beamforming vector from set of $\mathcal{R}$ o-RU to the $i^{th}$ UE served by $s^{th}$ slice that is the zero forcing beamforming vector to minimize the interference which is indicated as below
\begin{equation}
\bold{w}_{\mathcal{R},u(s,i)}^{k} = {\bold{h}_{\mathcal{R},u(s,i)}^{k}}({\bold{h}_{\mathcal{R},u(s,i)}^{H \: k}} {\bold{h}_{\mathcal{R},u(s,i)}^{k}})^{-1}
\end{equation}
Moreover, $BN_0$ denotes the power of Gaussian additive noise, and $I_{u(s,i)}^{k}$ is the power of interfering signals represented as follow
\begin{equation}
\begin{split}
I_{u(s,i)}^{k} &=
