...
Each slice $s_ \in \{1,23 \}$ consists of $K_{s}$ preallocated virtual resource blocks that are mapped to Physical Resource Blocks (PRBs), $M_s^{d}$ VNFs for the processing of O-DU, $M_s^{c}$ VNFs for the processing of O-CU-UP and $M_s^{u}$ VNFs for the processing of UPF.
...
In addition, there are $R$ single-antenna RU that are shared between slices. All RUs $r \in \{1,2,...,R \}$
transmit signals cooperatively to all the UEs(RUs are in comp mode). Moreover, all RUs, have access to PRBs.
