...
Next we introduce two methods which are both greedy algorithm.
The first method is a greedy algorithm that assign one O-RU to each UE and the second method is
a greedy algorithm that assign one or more than one O-RU to each UE and use coordinated multipoint (CoMP) transmissions \cite{yang2021multicast}.
\begin{enumerate}
\item\textit{Greedy Algorithm for Non-Comp O-RU Assignment:}
...
the problem \eqref{problem2} becomes to an NP-complete 0-1 multiple knapsack problem.  
...
\end{algorithmic}
\end{algorithm}
\item\textit{Greedy Algorithm for Comp O-RU Assignment:}
Here we want to solve problem \eqref{problem2}, by assuming that one or more than one O-RU can assign to each UE.
We solve this problem using GACA algorithm which used GAAOU algorithm.
In this algorithm, Firstly we sort slices based on their priority (So the UEs of slices are sorted too).
We depict $ {\mathfrak{B}}^{rem}_{u_{(s,i)}} \forall s, \forall i$ as a set of O-RUs for each user in each slice. At the beginning of the algorithm, $ {\mathfrak{B}}^{rem}_{u_{(s,i)}} =  \mathcal{R} \forall s, \forall i$. 
Then we assign one O-RU to each UE based on GAAOU algorithm so we guarantee that each UE is assigned to a O-RU. Next, we want to check if we can assign more than one O-RU to UE.


\begin{algorithm}
\caption{Greedy Algorithm for Comp Assignment of O-RU to UEs (GACA)}\label{alg3}
\begin{algorithmic}[1]
\State Set $g^r_{u(s,i)} = 0, \quad \forall s, \forall i, \forall r$.\label{a1}
\State Set $\mathfrak{C}_r = \mathfrak{t}_r, \forall r$  \label{a2}
\State Set ${\mathfrak{B}}^{rem}_{u_{(s,i)}} = \mathcal{R}$  $\forall s, \forall i$
\State Set $\mathtt{R}^{t} = 0$
\State Sort slices according to their priority factor ($\delta_s$) in descending order
\For {$s \gets 1$ to $S$}\label{33}
\For {$i \gets 1$ to $U_s$}
\State $RU = 0$
...
\State Set $g^{r^*}_{u(s,i)} = 1$ 
\State Set  $\mathfrak{C}_{r^*} = \mathfrak{C}_{r^*} - \psi_{{r^*},u(s,i)}$
\State Set $RU = 1$ and  ${\mathfrak{B}}^{rem}_{u_{(s,i)}} = \mathcal{R} \setminus \{{r^*}\} $
\State Update $I^{k}_{r,u(s,i)}, \forall s, \forall i$
\State Update $\bar{\mathcal{R}}^{r}_{u_{(s,i)}} \forall s, \forall i$
\Else
\State  ${\mathfrak{B}}^{rem}_{u_{(s,i)}} = \mathcal{R} \setminus \{{r^*}\} $
\EndIf
\State \textbf{end if}
...
\State  ${\mathfrak{B}}^{rem}_{u_{(s,i)}} = \mathcal{R} \setminus \{{r^*}\} $
\If{$\mathfrak{C}_{r^*} \geq \psi_{r^*,u(s,i)}$} 
\State Set $\mathtt{R}^{t} =\sum_{s=1}^S\sum_{i=1}^{U_s}\sum_{r=1}^{R}\delta_s g^{r}_{u(s,i)} \bar{\mathcal{R}}^{r^*}_{u_{(s,i)}}$
\State Update $I^{k}_{r,u(s,i)}, \forall s, \forall i$
\State Update $\bar{\mathcal{R}}^{r}_{u_{(s,i)}} \forall s, \forall i$
\State $g^{r^*}_{u(s,i)} = 1$ 
\State Set $\mathtt{R} =\sum_{s=1}^S\sum_{i=1}^{U_s}\sum_{r=1}^{R}\delta_s g^{r}_{u(s,i)}\bar{\mathcal{R}}^{r^*}_{u_{(s,i)}}$
\If{$\mathtt{R} \leq \mathtt{R}^{t}$}
\State Set $g^{r^*}_{u(s,i)} = 0$
\Else
\State update $\mathtt{R}^{t}$
\EndIf
\State \textbf{end if}
\EndIf
\State \textbf{end if}
