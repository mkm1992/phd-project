...
To convexify \eqref{eq11}, we replace ${\rho}_{r,u(s,i)}^{k}$ with $\hat{\rho}_{r,u(s,i)}^{k}$ in \eqref{shortPacket}. So, a lower bound for \eqref{eq11} is given that is a concave function.

Suppose UPF, O-CU and O-DU have the same processor 
\begin{lemma}
In problem \eqref{problem}, 
\end{lemma}
\bibliography{ref}
\end{document} 