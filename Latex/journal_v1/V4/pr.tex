...
\begin{alignat}{4}
\max\limits_{\boldsymbol{P}, \boldsymbol{E}, \boldsymbol{M}, \boldsymbol{G} }   \quad &  \sum_{s=1}^{S}\sum_{i=1}^{U_s}\bar{\mathcal{R}}_{u_{(s,k)}} \ \\
...
\begin{alignat}{4}
\max\limits_{ \boldsymbol{G} }   \quad &  \sum_{s=1}^S\sum_{i=1}^{U_s}\sum_{r=1}^{R}  g^r_{u(s,i)}\bar{\mathcal{R}}^r_{u_{(s,k)}} \ \\
...
To simplicated  and reformulated to the multiple knapsack problem suppose that each UE in each service is associated to only one O-RU. Therefore, constraint \eqref{p12} is restricted to equality
($\sum_{r=1}^R g^r_{u(s,i)} = 1 $). By changing this constraint,
the problem \eqref{problem2} convert to an NP-complete 0-1 multiple knapsack problem.  
...
Then we define ${RU}_{resid} = \mathcal{R} $ and $ \mathfrak{C}_r = \mathfrak{t}_r, \forall r$
as a set of O-RUs and value of each O-RU, respectively.
Afterward, we assign the O-RU that provides the highest achievable rate for each UE on the condition that 
...
The complexity of this algorithm is about $O(N\times \mathfrak{N})$
\begin{algorithm}
...
\State Set ${RU}_{resid} = \mathcal{R} $ and $\mathfrak{C}_r = \mathfrak{t}_r, \forall r$  \label{32}
\For {$s \gets 1$ to $S$}\label{33}
\For {$i \gets 1$ to $U_s$}
\For {$r \gets 1$ to $R$}
\State Acquire $\mathfrak{G}_r = \bar{\mathcal{R}}^r_{u_{(s,k)}}$
\EndFor
\State \textbf{end for}
\State Obtain $r = \text{argmax}_r \mathfrak{G}_r$
\While{$\mathfrak{C}_r \geq \psi_{r,u(s,i)}$}
\If{$\mathfrak{C}_r \geq \psi_{r,u(s,i)}$}
\State Set $g^r_{u(s,i)} = 1$ and $\mathfrak{C}_r = \mathfrak{C}_r - \psi_{r,u(s,i)}$
\Else
\State  ${RU}_{resid} = \mathcal{R} \setminus \{r\} $
\EndIf
\State \textbf{end if}
\EndWhile
...
\end{algorithmic}
\end{algorithm}
\item\textit{Lagrangian Function:} The second method to solve the O-RU assignment is to relax $\boldsymbol{G}$ between zero and one and solve the problem using Lagrangian function.
