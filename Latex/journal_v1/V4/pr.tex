...
Then we assign one O-RU to each UE based on GAAOU algorithm so we guarantee that each UE is assigned to a O-RU. Next, we want to assign more than one O-RU to UE.
\begin{algorithm}
...
\For {$i \gets 1$ to $Iter_{max}$}\label{33}
