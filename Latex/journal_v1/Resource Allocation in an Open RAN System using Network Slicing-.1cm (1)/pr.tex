...
Each pre-allocated slice contains reserved VNFs for the MAC/RLC functions in the O-DU logical node, the PDCP/SDAP functions in the O-CU-UP, and the UPF nodes. 
...
Each slice $s_j \in \{1,2,...,S_j \}$, $j \in \{1,2,3\}$ consists of 
%pre-allocated virtual resource blocks that are mapped to the Physical Resource Blocks (PRBs),
 $M_s^{d}$ VNFs for the processing of O-DU, $M_s^{c}$ VNFs for the processing of O-CU-UP and $M_s^{u}$ VNFs for the processing of UPF are required.
