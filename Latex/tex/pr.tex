...
single-antenna user equipments (UEs) that require certain service. Each slice $s \in \{1,2,...,S \}$ consists of $R_s$ RUs and $K_s$ physical resource blocks (PRBs), {\color{red} one DU and one CU}. All RUs in a slice, that is mapped to a service, transmit signals cooperatively to all the UEs in a specific service \cite{motalleb2017optimal,mimoCran}. Each RU $r \in \{1,2,...,R \}$ is mapped to a DU via an optical fiber link with limited fronthaul capacity.
...
Each packet is processed by one of the VNFs of a slice. So, the mean delay of the $s^{th}$ slice in the first and the second layer, modeled as M/M/1 queue, is formulated as $d_{s_1} = \frac{1}{\mu_1 - \alpha_{s}/{M_{s,1}}}$, and $d_{s_2} = \frac{1}{\mu_2 - \alpha_{s}/{M_{s,2}}},$ respectively.
where $1/\mu_1$ and $1/\mu_2$ are the mean service time of the first and the second layers respectively.%, due to the fact that the service time of each data packet has an exponential distribution for $\mu_i>\alpha_{s}/{M_{s,i}}$ $ i \in \{1,2\}$. 
...
the mean delay of the transmission layer is $d_{s_{tr}} = \frac{1}{R_{{tot}_s} - \alpha_{s}};$
...
&  p_{u_{(v,k)}}  \leq P_{max}^U  \quad \forall v, \forall k,\label{c122} \\
...
&p_{u_{(v,k)}}  \leq P_{max}^U  &&\quad \forall v, \forall k,  \\
...
Assume $\boldsymbol{\lambda}$, $\boldsymbol{\mu}$, $\boldsymbol{\chi}$, $\boldsymbol{\xi}$, and $\boldsymbol{ \kappa}$ are the matrix of Lagrangian multipliers that have non-zero positive elements.
...
&-\sum\limits_{v=1}^{V} \sum\limits_{k=1}^{U_v} \chi_{u_{(v,k)}} (p_{u_{(v,k)}}  - P_{max}^U)\\
...
\begin{equation}
p_{u(v,i)}^{*} = [\frac{\mathfrak{y}_{u(v,i)}\mathfrak{w}_{u(v,i)}-\mathfrak{x}_{u(v,i)}\mathfrak{z}_{u(v,i)}}{\mathfrak{x}_{u(v,i)}\mathfrak{w}_{u(v,i)} }-\chi_{u_{(v,k)}}]^+
\end{equation}
