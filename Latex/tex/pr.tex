...
\begin{equation*}
\phi_{tot} = \sum_{s=1}^{S}\sum_{d=1}^{D_c}x_{s,d}\phi
\end{equation*} 
Where, $x_{s,d}$ is a binary variable which indicates whether $d^{th}$ data-center is mapped to VNFs of $s^{th}$ slice or not.\\
...
\begin{subequations}
\begin{alignat}{4}
\max\limits_{\boldsymbol{P}, \boldsymbol{A}, \boldsymbol{X} }   \quad &   \eta(\boldsymbol{P},\boldsymbol{A})+\frac{1}{\phi_{tot}(\boldsymbol{X})} \\
\text{subject to} \quad  & \bar{p}_{r_{(s,i)}} \leq P_{max} && \quad \forall s, \forall i,   \\
&p_{u_{(v,k)}}  \geq 0  &&\quad \forall v, \forall k, \\
&\mathcal{R}_{u_{(v,k)}} \geq  \mathcal{R}_{u_{(v,k)}}^{max} && \quad \forall v, \forall k, \\                                 
&C_{r_{(s,i)}} \leq C_{r_{(s,i)}}^{max}  &&\quad \forall s, \forall i, \\
&D_{s} \leq D_{s}^{th}  &&\quad \forall s, \\
& \sum_{s=1}^{S}a_{vs} \geq 1 &&\quad \forall s, \\
& \sum_{d=1}^{D_c}\sum_{v=1}^{V}x_{s,d}a_{vs} \geq 1\times\sum_{v=1}^{V}a_{vs} &&\quad \forall s, \\
& \sum_{f=1}^{F_s}\Omega_{(f,s)} \leq  \sum_{d=1}^{D_c} x_{s,d} \tau_d                       && \quad \forall f, \forall s 
\end{alignat}
\label{constraints}
\end{subequations}
Where,$\boldsymbol{P} =[p_{u(v,k)}]  \forall v , \forall k $, $\boldsymbol{A} =[a_{vs}]  \forall v , \forall s $ and $\boldsymbol{X} =[x_{s,d}]  \forall s , \forall d $.
...
\begin{subequations}
\begin{alignat}{4}
\min\limits_{\boldsymbol{X} }   \quad &   \phi_{tot}(\boldsymbol{X})\\
\text{subject to} \quad  & \bar{p}_{r_{(s,i)}} \leq P_{max} && \quad \forall s, \forall i,   \\
& \sum_{s=1}^{S}a_{vs} \geq 1 &&\quad \forall s, \\
& \sum_{d=1}^{D_c}\sum_{v=1}^{V}x_{s,d}a_{vs} \geq 1\times\sum_{v=1}^{V}a_{vs} &&\quad \forall s, \\
& \sum_{f=1}^{F_s}\Omega_{(f,s)} \leq  \sum_{d=1}^{D_c} x_{s,d} \tau_d                       && \quad \forall f, \forall s 
...
In this part, we want to solve \eqref{constraints1}. Since the problem is non-convex, iterative algorithm is applied.
To solve the problem and obtain optimum $\boldsymbol{A}$ and $\boldsymbol{P}$ we divide problem \eqref{constraints1} to 
two different part that iteratively solve. 
Firstly, we want to obtain $\boldsymbol{A}$ by fixing $\boldsymbol{P} = \boldsymbol{P}_{max}$ in the problem \eqref{constraints1}. This part of problem is solved in two different method. The first method is using MOSEK and second method is a heuristic method.
...
\begin{algorithmic}[1]
\State Sort Services according to their priority and the number of Users in the services and their requirements.
\State Sort Slices according to the number of PRBs and number of RRHs and VMs  and their max of Capacities of resources. 
