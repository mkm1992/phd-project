...
Based on their type of request, UEs are classified into various services where UEs in the same service has similar service requests. In addition, each service is mapped to one or more slices based on the resource of the slices. Each VNF of the O-RAN is mapped to one or some virtual machines (VMs) in the data center. For simplicity of analysis and without loss of generality, we assume a VNF is mapped to one VM and requires specific physical resources including storage, memory, and processing \cite{frdl,luong2018novel,luong2018novel1}.
...
Assume $\boldsymbol{\lambda}$, $\boldsymbol{\chi}$, $\boldsymbol{\mu}$, $\boldsymbol{\xi}$, and $\boldsymbol{ \kappa}$ are the matrix of Lagrangian multipliers that have non-zero positive elements.
