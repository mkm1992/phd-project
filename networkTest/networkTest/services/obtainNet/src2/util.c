...
char *print_test_results(struct ntttcp_test_endpoint *tep)
{
	struct ntttcp_test_endpoint_results *tepr = tep->results;
	uint64_t total_bytes = tepr->total_bytes;
   ...
   fp = fopen("tmp/test.txt", "w+");
	...
	fprintf(fp,"test duration\t:%.2f seconds \n", test_duration);
	PRINT_INFO_FREE(log);
